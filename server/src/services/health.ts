...
import { notifyKeyStatusChange } from './notifications.js';
import type { Platform, KeyStatus } from '@zad/shared/types.js';
...
export async function checkKeyHealth(keyId: number): Promise<KeyStatus> {
  ...
  const prevStatus = String(row.status ?? 'unknown');
  ...
  try {
    ...
    if (prevStatus !== status) notifyKeyStatusChange(row.platform, prevStatus, status);
    ...
    db.prepare("UPDATE api_keys SET status = ?, last_checked_at = datetime('now') WHERE id = ?")
      .run('error', keyId);
    if (prevStatus !== 'error') notifyKeyStatusChange(row.platform, prevStatus, 'error');
    return 'error';
  }
}
