...
/** Add per-request detail columns (routing metadata) to the requests log. */
function migrateRequestsV1(db: Database.Database) {
  addColumnIfMissing(db, 'requests', 'requested_model', 'requested_model TEXT');
  addColumnIfMissing(db, 'requests', 'message_count', 'message_count INTEGER');
  addColumnIfMissing(db, 'requests', 'had_tools', 'had_tools INTEGER');
  addColumnIfMissing(db, 'requests', 'streamed', 'streamed INTEGER');
  addColumnIfMissing(db, 'requests', 'attempts', 'attempts INTEGER');
  addColumnIfMissing(db, 'requests', 'finish_reason', 'finish_reason TEXT');
}

/** Optional captured content (only populated when capture_content is on):
 *  the full request messages (for replay) and a truncated response preview. */
function migrateRequestsV2(db: Database.Database) {
  addColumnIfMissing(db, 'requests', 'request_messages', 'request_messages TEXT');
  addColumnIfMissing(db, 'requests', 'response_preview', 'response_preview TEXT');
}


/** Dynamically-added (OpenAI-compatible) providers, registered at boot. */
function ensureCustomProvidersTable(db: Database.Database) {
  db.exec(`
...
function ensureLocalProviders(db: Database.Database) {
  ...
  const models: Array<[string, string, string, number, number, string, number | null, number | null, number | null, number | null, string, number | null]> = [
    ['meridian', 'claude-opus-4-7', 'Claude Opus 4.7 (Meridian)', 1, 6, 'Local', null, null, null, null, 'local', 200000],
    ['meridian', 'claude-opus-4-6', 'Claude Opus 4.6 (Meridian)', 1, 6, 'Local', null, null, null, null, 'local', 200000],
    ['meridian', 'claude-sonnet-4-6', 'Claude Sonnet 4.6 (Meridian)', 2, 4, 'Local', null, null, null, null, 'local', 200000],
    ['meridian', 'claude-haiku-4-5', 'Claude Haiku 4.5 (Meridian)', 5, 2, 'Local', null, null, null, null, 'local', 200000],
    ['cliproxyapi', 'claude-sonnet-4-5', 'Claude Sonnet 4.5 (CLIProxyAPI)', 3, 5, 'Local', null, null, null, null, 'local', 200000],
    ['cliproxyapi', 'claude-haiku-4-5', 'Claude Haiku 4.5 (CLIProxyAPI)', 6, 3, 'Local', null, null, null, null, 'local', 200000],
    ['cliproxyapi', 'gemini-2.5-pro', 'Gemini 2.5 Pro (CLIProxyAPI)', 4, 5, 'Local', null, null, null, null, 'local', 1048576],
    ['cliproxyapi', 'gemini-2.5-flash', 'Gemini 2.5 Flash (CLIProxyAPI)', 8, 2, 'Local', null, null, null, null, 'local', 1048576],
    ['cliproxyapi', 'openai-gpt-4o', 'GPT-4o (CLIProxyAPI)', 6, 5, 'Local', null, null, null, null, 'local', 128000],
    ['cliproxyapi', 'openai-gpt-4o-mini', 'GPT-4o Mini (CLIProxyAPI)', 11, 2, 'Local', null, null, null, null, 'local', 128000],
    ['cliproxyapi', 'qwen3-4b', 'Qwen3 4B (CLIProxyAPI)', 14, 1, 'Local', null, null, null, null, 'local', 32768],
  ];

  // The meridian / cliproxyapi providers are env-gated (off unless the matching
  // base URL is configured). Only seed their models when the provider is enabled,
  // so the catalog never drifts ahead of the registered providers.
  const sync = db.transaction(() => {
    for (const m of models) {
      if (m[0] === 'meridian' && !process.env.MERIDIAN_BASE_URL) continue;
      if (m[0] === 'cliproxyapi' && !process.env.CLIPROXY_BASE_URL) continue;
      upsert.run(...m);
      const row = findModel.get(m[0], m[1]) as { id: number } | undefined;
...
function ensureUnifiedKey(db: Database.Database) {
  ...
  if (!existing) {
    const key = `zad-${crypto.randomBytes(24).toString('hex')}`;
...
/** Generic key/value settings accessors (settings table). */
