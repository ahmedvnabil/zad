...
import { z } from 'zod';
import type { ChatMessage } from '@zad/shared/types.js';
import { routeRequest, recordRateLimitHit, recordSuccess, type RouteResult } from '../services/router.js';
import { recordRequest, recordTokens, setCooldown } from '../services/ratelimit.js';
import { getDb, getUnifiedApiKey, getSetting } from '../db/index.js';
import { getModelMetadataSnake } from '../lib/capabilities.js';
import { notifyChange } from '../lib/events.js';

export const proxyRouter = Router();

// Virtual "auto" model. Clients like Hermes require a non-empty `model` field
// on every request, but zad's whole point is to pick the model itself.
// Requesting this id means "let the router decide" — identical to omitting
// `model` entirely.
const AUTO_MODEL_ID = 'auto';

// Capability/ordering profiles for the virtual "auto" model. Clients can ask
// for "best available model that does X" instead of pinning a specific id.
const AUTO_PROFILES: Record<string, { requiredCaps?: { tools?: boolean; vision?: boolean }; orderBy?: 'intelligence' | 'speed' | 'cheap'; desc: string }> = {
  tools: { requiredCaps: { tools: true }, desc: 'Auto, restricted to tool-calling models' },
  vision: { requiredCaps: { vision: true }, desc: 'Auto, restricted to image-input models' },
  smart: { orderBy: 'intelligence', desc: 'Auto, ordered by intelligence (best first)' },
  fast: { orderBy: 'speed', desc: 'Auto, ordered by speed (fastest first)' },
  cheap: { orderBy: 'cheap', desc: 'Auto, ordered by cheapest equivalent paid price' },
};

// Parse the profile suffix of an `auto[:profile]` model id. Returns {} for plain
// "auto" or an unknown profile; null when the id isn't an auto model at all.
function parseAutoProfile(modelId: string | undefined): { requiredCaps?: { tools?: boolean; vision?: boolean }; orderBy?: 'intelligence' | 'speed' | 'cheap' } | null {
  if (!modelId) return null;
  const id = modelId.toLowerCase();
  if (id === AUTO_MODEL_ID) return {};
  if (!id.startsWith('auto:')) return null;
  return AUTO_PROFILES[id.slice(5)] ?? {};
}
    ...
    data: [
      {
        ...
        features: ['auto-routing', 'chat-completions', 'tool-calling'],
      },
      // Virtual auto profiles — "best available model that matches X".
      ...Object.entries(AUTO_PROFILES).map(([key, profile]) => ({
        id: `${AUTO_MODEL_ID}:${key}`,
        object: 'model',
        created: 0,
        owned_by: 'zad',
        name: profile.desc,
        context_window: null,
        input_modalities: profile.requiredCaps?.vision ? ['text', 'image'] : ['text'],
        output_modalities: ['text'],
        capabilities: {
          text: true,
          tool_calling: profile.requiredCaps?.tools ?? true,
          image_input: profile.requiredCaps?.vision ?? false,
          image_output: false,
          audio_input: false,
          audio_output: false,
        },
        features: ['auto-routing', 'chat-completions', `profile:${key}`],
      })),
      ...models.map(m => {
        const metadata = getModelMetadataSnake(m.platform, m.model_id);
        return {
          id: m.model_id,
          object: 'model',
          created: 0,
...
proxyRouter.post('/chat/completions', async (req: Request, res: Response) => {
  ...
  const autoProfile = parseAutoProfile(requestedModel);
  if (autoProfile) {
    // "auto" or "auto:<profile>" → behave like an omitted model field, with
    // optional capability/ordering constraints applied below.
  ...
  // Capability-aware routing: if the request carries function-calling tools,
  // only route to models that can actually do tool calling (skip known-incapable
  // ones instead of failing on them). `tool_choice: 'none'` opts out. An
  // `auto:<profile>` model can also force a capability or reorder the chain.
  const requiresTools = ((tools?.length ?? 0) > 0 && tool_choice !== 'none') || autoProfile?.requiredCaps?.tools === true;
  const requiresVision = autoProfile?.requiredCaps?.vision === true;
  const requiredCaps = (requiresTools || requiresVision)
    ? { ...(requiresTools ? { tools: true } : {}), ...(requiresVision ? { vision: true } : {}) }
    : undefined;
  const orderBy = autoProfile?.orderBy;

  // Per-request metadata captured in the request log for the detail view.
  // Content (full messages + response preview) is only captured when the
  // `capture_content` setting is on — off by default for privacy/size.
  const captureContent = getSetting('capture_content') === 'true';
  const reqMeta: RequestMeta = {
    requestedModel: requestedModel ?? 'auto',
    messageCount: messages.length,
    hadTools: requiresTools,
    streamed: !!stream,
    requestMessages: captureContent ? JSON.stringify(messages).slice(0, 100_000) : null,
  };
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      route = routeRequest(estimatedTotal, skipKeys.size > 0 ? skipKeys : undefined, preferredModel, requiredCaps, orderBy);
    } catch (err: any) {
    ...
    try {
      if (stream) {
        ...
        try {
          ...
          for await (const chunk of gen) {
            ...
            if (captureContent && previewBuf.length < 2000) previewBuf += text;
          ...
          logRequest(route.platform, route.modelId, 'success', estimatedInputTokens, totalOutputTokens, Date.now() - start, null, { ...reqMeta, attempts: attempt + 1, finishReason: 'stop', responsePreview: captureContent ? previewBuf.slice(0, 2000) : null });
          return;
        } catch (streamErr: any) {
          if (streamStarted) {
            ...
            logRequest(route.platform, route.modelId, 'error', estimatedInputTokens, totalOutputTokens, Date.now() - start, streamErr.message, { ...reqMeta, attempts: attempt + 1 });
            return;
          }
        ...
        const respContent = (result as any).choices?.[0]?.message?.content;
        logRequest(
          ...
          Date.now() - start, null,
          {
            ...
            responsePreview: captureContent && typeof respContent === 'string' ? respContent.slice(0, 2000) : null,
          },
        );
        return;
      }
      ...
      logRequest(route.platform, route.modelId, 'error', estimatedInputTokens, 0, latency, err.message, { ...reqMeta, attempts: attempt + 1 });
...
interface RequestMeta {
  ...
  requestMessages?: string | null; // full messages JSON, only when capture is on
  responsePreview?: string | null; // truncated response text, only when capture is on
}
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO requests (
        ...
        requested_model, message_count, had_tools, streamed, attempts, finish_reason,
        request_messages, response_preview
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      meta.hadTools === undefined ? null : (meta.hadTools ? 1 : 0),
      meta.streamed === undefined ? null : (meta.streamed ? 1 : 0),
      meta.attempts ?? null,
      meta.finishReason ?? null,
      meta.requestMessages ?? null,
      meta.responsePreview ?? null,
    );
    // Notify live dashboards (SSE) that a new request landed.
