...
describe('Full Integration Flow', () => {
  ...
  it('Step 12: Explicit disabled model returns 400 with disabled reason', async () => {
    // zai-glm-4.7 (cerebras) is disabled (V5 migration) and — unlike
    // gemini-2.5-pro, which the local CLIProxyAPI provider now re-adds as an
    // enabled row — exists on exactly one platform, so it stays globally
    // disabled. Use it as the known-disabled fixture.
    const { status, body } = await req(app, 'POST', '/v1/chat/completions', {
      model: 'zai-glm-4.7',
      messages: [{ role: 'user', content: 'hi' }],
    }, authHeaders());
  ...
  it('Step 13: auto-optimize setting round-trips', async () => {
    const off = await req(app, 'GET', '/api/settings/auto-optimize', undefined, authHeaders());
    expect(off.body.enabled).toBe(false);

    const on = await req(app, 'PUT', '/api/settings/auto-optimize', { enabled: true }, authHeaders());
    expect(on.body.enabled).toBe(true);

    const check = await req(app, 'GET', '/api/settings/auto-optimize', undefined, authHeaders());
    expect(check.body.enabled).toBe(true);
  });

  it('Step 14: request log endpoint returns a paginated shape', async () => {
    const { status, body } = await req(app, 'GET', '/api/analytics/requests?limit=10', undefined, authHeaders());
    expect(status).toBe(200);
    ...
    if (body.requests.length > 0) {
      ...
      for (const k of ['totalTokens', 'estimatedCostUsd', 'attempts', 'requestedModel', 'hadTools', 'streamed', 'finishReason', 'promptPreview', 'responsePreview', 'requestMessages']) {
        expect(r).toHaveProperty(k);
      }
    }
  });

  it('Step 15: capture-content setting round-trips and clear works', async () => {
    const off = await req(app, 'GET', '/api/settings/capture-content', undefined, authHeaders());
    expect(off.body.enabled).toBe(false);

    const on = await req(app, 'PUT', '/api/settings/capture-content', { enabled: true }, authHeaders());
    expect(on.body.enabled).toBe(true);

    const cleared = await req(app, 'POST', '/api/settings/clear-request-content', undefined, authHeaders());
    expect(cleared.body).toHaveProperty('cleared');
  });

  it('Step 16: request log supports status filter + export', async () => {
    const filtered = await req(app, 'GET', '/api/analytics/requests?status=success&limit=5', undefined, authHeaders());
    expect(filtered.status).toBe(200);
    expect(Array.isArray(filtered.body.requests)).toBe(true);
    for (const r of filtered.body.requests) expect(r.status).toBe('success');

    const csv = await req(app, 'GET', '/api/analytics/requests/export?format=csv', undefined, authHeaders());
